#!/usr/bin/env bash
# YOLO Python Canvas 完整链路启动脚本。
# 启动顺序固定为 camera_web_cpp -> yolo_web_py_canvas，并在每一步后检查 /health。
set -Eeuo pipefail
...
YOLO_SCRIPT=${YOLO_WS}/scripts/board/start_yolo_web_py_canvas.sh
CAMERA_LOG=/tmp/camera_web_cpp.log
YOLO_LOG=/tmp/yolo_web_py_canvas.log
CAMERA_HEALTH=http://127.0.0.1:8081/health
YOLO_HEALTH=http://127.0.0.1:8091/health
CAMERA_ARGS=("$@")
...
wait_for_http() {
  ...
  # Python 推理启动较慢，等待窗口比 C++ 版略长。
  for _ in $(seq 1 45); do
    if curl -fsS --max-time 2 "${url}" >/tmp/yolo_web_py_canvas_health.txt 2>/dev/null; then
      local health
      health=$(cat /tmp/yolo_web_py_canvas_health.txt)
      local frames
...
main() {
  ...
  if [[ ! -x "${YOLO_SCRIPT}" ]]; then
    echo "YOLO Python Canvas script not found or not executable: ${YOLO_SCRIPT}" >&2
    exit 1
  fi
  stop_old_processes
  : >"${CAMERA_LOG}"
  ...
  echo "Starting Python YOLO Canvas overlay service on 8091..."
  nohup "${YOLO_SCRIPT}" >"${YOLO_LOG}" 2>&1 &
  yolo_pid=$!
  wait_for_http "yolo_web_py_canvas" "${YOLO_HEALTH}" "${YOLO_LOG}" 1
  echo "All C++ camera + Python YOLO Canvas overlay services started."
  echo "Camera page: http://127.0.0.1:8081/"
  echo "YOLO Python Canvas page: http://127.0.0.1:8091/"
  echo "Logs:"
  echo "  camera pid=${camera_pid}, log=${CAMERA_LOG}"
  echo "  yolo   pid=${yolo_pid}, log=${YOLO_LOG}"
}
