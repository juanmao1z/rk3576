...
#include "yolo_web_cpp/yolo_overlay_node.hpp"
...
{
  // labels:=coco 是通用工作区的快捷模式；其他值按逗号分隔解析。
  std::string normalized = trim(value);
  std::transform(normalized.begin(), normalized.end(), normalized.begin(), [](unsigned char ch) {
    return static_cast<char>(std::tolower(ch));
  });
  if (normalized == "coco") {
    return parse_labels(kCocoLabels);
  }
...
YoloOverlayNode::YoloOverlayNode()
: Node("yolo_web_cpp_node")
{
  ...
  labels_ = parse_labels(declare_parameter<std::string>("labels", "coco"));
  ...
  subscription_ = create_subscription<sensor_msgs::msg::CompressedImage>(
    ...
    std::bind(&YoloOverlayNode::on_image, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(
    get_logger(),
    "Serving YOLO C++ overlay at http://0.0.0.0:%d/ using %s; publishing detections on %s",
    port_,
    model_path_.c_str(),
...
void YoloOverlayNode::on_image(const sensor_msgs::msg::CompressedImage::SharedPtr message)
{
  try {
    ...
    RCLCPP_ERROR_THROTTLE(
      ...
      *get_clock(),
      2000,
      "YOLO C++ callback failed: %s",
      ex.what());
  }
}

void YoloOverlayNode::update_snapshot_and_publish(
...
void YoloOverlayNode::publish_detections(
  const sensor_msgs::msg::CompressedImage & input,
...
DetectionSnapshot YoloOverlayNode::snapshot() const
{
  std::lock_guard<std::mutex> lock(snapshot_mutex_);
...
double YoloOverlayNode::current_result_fps_locked() const
{
