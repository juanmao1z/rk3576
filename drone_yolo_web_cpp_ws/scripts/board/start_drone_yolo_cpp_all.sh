#!/usr/bin/env bash
# 无人机 YOLO C++ Canvas 完整链路启动脚本。
# 启动顺序固定为 camera_web_cpp -> drone_yolo_web_cpp，并在每一步后检查 /health。
set -Eeuo pipefail
...
stop_old_processes() {
  ...
  pkill -TERM -f '[r]os2 launch drone_yolo_web_cpp' || true
  pkill -TERM -f '[d]rone_yolo_web_cpp_node' || true
...
wait_for_http() {
  ...
  for _ in $(seq 1 30); do
    if curl -fsS --max-time 2 "${url}" >/tmp/drone_yolo_web_cpp_health.txt 2>/dev/null; then
      local health
      health=$(cat /tmp/drone_yolo_web_cpp_health.txt)
      local frames
...
main() {
  ...
  if [[ ! -x "${YOLO_CPP_SCRIPT}" ]]; then
    echo "Drone YOLO C++ script not found or not executable: ${YOLO_CPP_SCRIPT}" >&2
    exit 1
  fi
  ...
  echo "Starting Drone YOLO C++ overlay service on 8092..."
  nohup "${YOLO_CPP_SCRIPT}" >"${YOLO_CPP_LOG}" 2>&1 &
  yolo_pid=$!
  wait_for_http "drone_yolo_web_cpp" "${YOLO_CPP_HEALTH}" "${YOLO_CPP_LOG}" 1

  echo "All C++ camera + Drone YOLO overlay services started."
  echo "Camera page: http://127.0.0.1:8081/"
  echo "Drone YOLO C++ overlay page: http://127.0.0.1:8092/"
  echo "Logs:"
