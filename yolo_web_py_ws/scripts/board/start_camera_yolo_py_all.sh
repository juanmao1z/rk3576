#!/usr/bin/env bash
# YOLO Python 服务端画框完整链路启动脚本。
# 启动顺序固定为 camera_web_cpp -> yolo_web_py，并在每一步后检查 /health。
set -Eeuo pipefail
...
YOLO_PY_SCRIPT=${YOLO_WS}/scripts/board/start_yolo_web_py.sh
CAMERA_LOG=/tmp/camera_web_cpp.log
YOLO_PY_LOG=/tmp/yolo_web_py.log
CAMERA_HEALTH=http://127.0.0.1:8081/health
YOLO_PY_HEALTH=http://127.0.0.1:8090/health
CAMERA_ARGS=("$@")
...
wait_for_http() {
  ...
  for _ in $(seq 1 45); do
    if curl -fsS --max-time 2 "${url}" >/tmp/yolo_web_py_health.txt 2>/dev/null; then
      local health
      health=$(cat /tmp/yolo_web_py_health.txt)
      local frames
...
main() {
  ...
  if [[ ! -x "${YOLO_PY_SCRIPT}" ]]; then
    echo "YOLO Python script not found or not executable: ${YOLO_PY_SCRIPT}" >&2
    exit 1
  fi
  ...
  echo "Starting Python YOLO result stream on 8090..."
  nohup "${YOLO_PY_SCRIPT}" >"${YOLO_PY_LOG}" 2>&1 &
  yolo_pid=$!
  wait_for_http "yolo_web_py" "${YOLO_PY_HEALTH}" "${YOLO_PY_LOG}" 1

  echo "All C++ camera + Python YOLO result-stream services started."
  echo "Camera page: http://127.0.0.1:8081/"
  echo "YOLO Python result page: http://127.0.0.1:8090/"
  echo "Logs:"
  echo "  camera pid=${camera_pid}, log=${CAMERA_LOG}"
  echo "  yolo   pid=${yolo_pid}, log=${YOLO_PY_LOG}"
}
