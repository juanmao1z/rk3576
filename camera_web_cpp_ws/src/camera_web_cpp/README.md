...
这个包的目标是把 USB 摄像头输出的原生 MJPEG 帧尽量少搬运、少处理地送到浏览器，同时把同一份压缩图像发布给后续 YOLO 节点使用。
...
摄像头本身已经输出 MJPEG，也就是一帧一帧的 JPEG 压缩数据。这个 C++ 包不会把帧解码成 OpenCV 图像，也不会再执行 JPEG 编码。
...
因此本包没有 `jpeg_quality` 参数。如果需要改变图像数据量，使用摄像头采集分辨率或帧率参数。当前默认是 `640x480@25`，切到 `1280x720` 时脚本会自动请求 `30 FPS`。
